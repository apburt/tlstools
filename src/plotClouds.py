...
def plotClouds(clouds,azimuth=30,elevation=30,outname='clouds.pdf',disp_axes=False):

	fig = matplotlib.pyplot.figure(frameon=False)
	ax = fig.add_subplot(111, projection='3d')
	ax.view_init(azim=azimuth,elev=elevation)
	for i in range(len(clouds)):
		cloud = numpy.loadtxt(clouds[i])
		if(len(clouds) == 1):
			colour = 'k'
		else:
...
if __name__ == "__main__":
	...
	parser.add_argument('-c','--clouds',nargs='*',help='ASCII xyx clouds')
	...
	plotClouds(args.clouds,azimuth=args.azimuth,elevation=args.elevation,outname=args.outname,disp_axes=args.dispaxes)
